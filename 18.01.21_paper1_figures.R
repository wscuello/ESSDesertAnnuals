# Dependencies: 18.01.21_paper1.R,  18.01.21_paper1_calledon.R

source('18.01.21_paper1.R')
...
difference_in_the_two_linear_models = 1.15+.42*stan_beta1s + .1*stan_beta2s + .13*stan_sold
difference_of_logit_data_points = (logit(ess.vec.baseline) - logit(use_table$fieldESS))
plot(difference_in_the_two_linear_models, difference_of_logit_data_points, cex = 2, lwd =3, col = the_colors, xlim = c(0.5, 2), ylim = c(min(difference_of_logit_data_points), max(difference_of_logit_data_points)), ylab = 'logit(ESS) - logit(observed)', xlab = 'Model (2) - Model (3)')
text(difference_in_the_two_linear_models, difference_of_logit_data_points, labels = row.names(use_table), pos = c(4,3,1,2,4,4,2,3,4,1), col = the_colors)
abline(lm(difference_of_logit_data_points~difference_in_the_two_linear_models))
abline(c(0,0), c(1,1), lwd = 2, lty = 2)

pdf('Figure_exploration0_paper1.pdf')
{
  morphed_1_linear_model = .86 - .10*stan_beta1s - .29*stan_beta2s - .57*stan_sold
...
{
  morphed_1_linear_model = -.29 - .10*stan_beta1s - .29*stan_beta2s - .57*stan_sold
  plot(logit(predicting_ess), logit(use_table$fieldESS), cex = 2, col = the_colors, lwd = 3, ylim = c(-3,2.5), xlim = c(-2.5, 1.5), xlab='pred (circle) & ess (triangle)', ylab = 'observed', main = 'estimates on logit scale')
  text(logit(predicting_ess), logit(use_table$fieldESS), labels = row.names(use_table), pos = c(4,3,1,2,4,4,2,3,4,1), col = the_colors)
  lines(morphed_1_linear_model, logit(use_table$fieldESS), type = 'p', pch = 2, cex =2, col = rgb(0,0,0,alpha=0.5), lwd =2)
  text(morphed_1_linear_model, logit(use_table$fieldESS), labels = row.names(use_table), pos = c(4,3,1,2,4,4,2,3,4,1), col = rgb(0,0,0,alpha=0.7))
...
{
  morphed_2_linear_model = -.29 - .52*stan_beta1s - .29*stan_beta2s - .57*stan_sold
  plot(logit(predicting_ess), logit(use_table$fieldESS), cex = 2, col = the_colors, lwd = 3, ylim = c(-3,2.5), xlim = c(-2.5, 1.5), xlab='pred (circle) & ess (triangle)', ylab = 'observed', main = 'estimates on logit scale')
  text(logit(predicting_ess), logit(use_table$fieldESS), labels = row.names(use_table), pos = c(4,3,1,2,4,4,2,3,4,1), col = the_colors)
...
{
  morphed_3_linear_model = -.29 - .52*stan_beta1s - .29*stan_beta2s - .70*stan_sold
  plot(logit(predicting_ess), logit(use_table$fieldESS), cex = 2, col = the_colors, lwd = 3, ylim = c(-3,2.5), xlim = c(-2.5, 1.5), xlab='pred (circle) & ess (triangle)', ylab = 'observed', main = 'estimates on logit scale')
  text(logit(predicting_ess), logit(use_table$fieldESS), labels = row.names(use_table), pos = c(4,3,1,2,4,4,2,3,4,1), col = the_colors)
...
{
  morphed_4_linear_model = -.29 - .52*stan_beta1s - .39*stan_beta2s - .70*stan_sold
  plot(logit(predicting_ess), logit(use_table$fieldESS), cex = 2, col = the_colors, lwd = 3, ylim = c(-3,2.5), xlim = c(-2.5, 1.5), xlab='pred (circle) & ess (triangle)', ylab = 'observed', main = 'estimates on logit scale')
  text(logit(predicting_ess), logit(use_table$fieldESS), labels = row.names(use_table), pos = c(4,3,1,2,4,4,2,3,4,1), col = the_colors)
...
par(mar=c(7,7,2,1))
plot(log_precip_plus1, replace_or_not_vec, ylab = 'Logistic', xlab = 'log(Precip + 1)', main = 'ERLA', cex = 4, cex.axis = 2.5, cex.lab = 3, cex.main = 2, lwd = 1, col = "#00008B")
domain_for_logistic = seq(min(log_precip_plus1), max(log_precip_plus1), by = .1)
temp_logistic_func = function(beta0, beta1, x){return(1/(1+exp(-(beta0 + beta1*x))))}
lines(domain_for_logistic, temp_logistic_func(coef(logistic_hockey)[[1]],coef(logistic_hockey)[[2]],domain_for_logistic), col = "#00008B", lwd = 5)
mtext(paste0('B'), side = 1, adj = .95, line = -9.0, cex = 3.5)
dev.off()

pdf('Figure1C.pdf', height = 7, width = 7)
par(mar=c(7,7,2,1))
plot(log_precip_plus1[which_log_ks_to_regress], log_k_vec[which_log_ks_to_regress], ylim = c(-1, 8), xlim = c(-.5, 5), xlab = 'log(Precip + 1)', ylab = 'log(Yield + .5)', main = 'ERLA', cex = 4, cex.main = 2, cex.lab = 3, cex.axis = 2.5, lwd = 1, col = "#00008B")
domain = seq(-5, 5, by = .01)
abline(conditioned_ls, lty = 1, lwd = 5, col = "#00008B")
abline(v = 0, h = 0, lty = 2, lwd = 3)
mtext(paste0('C'), side = 1, adj = .95, line = -9.0, cex = 3.5)
dev.off()

pdf('Figure2A.pdf', width = 8, height = 8)
{par(mar=c(7,7,2,1))
plot(ess.vec.baseline, use_table$fieldESS, ylim = c(0,1), xlim = c(0,1), xlab = 'Estimated ESS', ylab = 'Observed Germination Fractions', cex.lab = 2, cex.axis = 1.5, cex = 4, col = the_colors, lwd = 2)
...
{par(mar=c(7,7,2,1))
plot(predicting_ess, use_table$fieldESS, col = the_colors, cex = 4, cex.lab= 2, cex.axis = 1.5, ylab = 'Observed Germination Fractions', xlab = expression('logit'^-1*'(L'[obs]*')'), ylim = c(0,1), xlim = c(0,1), lwd = 2)
